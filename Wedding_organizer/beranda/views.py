...
def daftar_gedung_view(request):
    ...
    """
    # REVISI: Hanya tampilkan gedung yang punya pemilik (wo__isnull=False) 
    # DAN statusnya AKTIF (is_active=True)
    gedungs = Gedung.objects.filter(wo__isnull=False, is_active=True).order_by('-created_at')
    ...
    # Pencarian berdasarkan Nama Jalan / Alamat
    if jalan:
...
def detail_wo_view(request, wo_id):
    ...
    # Hanya tampilkan gedung yang aktif di profil WO juga
    gedungs = Gedung.objects.filter(wo=profil_wo.user, is_active=True).order_by('nama_gedung')
    ...
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        if form.is_valid():
            # Logika simpan ulasan ada di sini jika diperlukan,
            # tapi kita utamakan lewat 'status pesanan' agar terhubung ke transaksi.
            pass
...
def detail_gedung_view(request, gedung_id):
    ...
    # Opsional: Jika gedung non-aktif diakses via link langsung, bisa redirect atau tampilkan pesan
    if not gedung.is_active:
        return redirect('daftar_gedung')
