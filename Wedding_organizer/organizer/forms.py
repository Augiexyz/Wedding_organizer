...
class PaketForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['nama_paket'].widget.attrs.update({'class': input_field_class, 'placeholder': 'Contoh: Paket Diamond'})
        self.fields['harga'].widget.attrs.update({'class': input_field_class, 'placeholder': 'Contoh: 50000000'})
        self.fields['deskripsi'].widget.attrs.update({'class': f'{input_field_class} h-40', 'placeholder': 'Detail layanan...'})
        self.fields['foto'].widget.attrs.update({'class': 'block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100'})
        self.fields['is_active'].widget.attrs.update({'class': 'h-5 w-5 rounded text-pink-600 focus:ring-pink-500'})
...
# --- FORM PESANAN (LOGIKA FILTER GEDUNG) ---
class PesananForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        # Ambil data 'paket' yang dikirim dari views.py
        ...
        if paket:
            if paket.kategori_gedung == 'non_gedung':
                # Jika paket tidak butuh gedung, sembunyikan dropdown
                ...
                # Jika paket butuh gedung, filter sesuai WO dan Kategori Paket
                ...
                # Matikan input manual lokasi karena akan otomatis pakai alamat gedung
                self.fields['lokasi_acara'].required = False
                self.fields['lokasi_acara'].widget.attrs.update({
                    'placeholder': 'Lokasi otomatis terisi sesuai gedung yang dipilih',
                    'disabled': 'disabled',
...
class GedungForm(forms.ModelForm):
    class Meta:
        model = Gedung
        fields = ['nama_gedung', 'lokasi', 'kapasitas', 'kategori', 'harga_sewa', 'deskripsi', 'fasilitas', 'foto_gedung', 'is_active']
        ...
        widgets = {
            ...
            'is_active': forms.CheckboxInput(attrs={'class': 'h-5 w-5 rounded text-pink-600 focus:ring-pink-500 cursor-pointer'}),
        }
        ...
        labels = {
            ...
            'is_active': 'Gedung Tersedia (Aktif)'
        }
...
class UlasanForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'rating': forms.Select(attrs={'class': 'input-field w-full md:w-auto'}), 
